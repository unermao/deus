...
public class StreamingPeer extends Peer {
		...
		if( this.getNeighbors().size() < this.getMaxPartnersNumber() ){
			...
			while( count < 20 ){
				...
				if(
						...
						&& !this.neighbors.contains(peer) //Se non e' giˆ tra i miei vicni
	...
	public boolean findProviderNodeFromLastSegment() {
		...
		if( this.getSourceStreamingNode() == null && this.getServerNode() == null )
		{
			...
			{
				//Cerco all'interno della mia lista di vicini se trovo un fornitore partendo dal segmento che giˆ posseggo
				for(int i = 0 ; i < this.getNeighbors().size(); i++){
