 ...
 *
 */
public class StreamingFreeResourceEvent extends Event {

	private Node resOwner = null;
	private String resName = null;
	private int resAmount = 0;

	public StreamingFreeResourceEvent(String id, Properties params,
	...
	public Object clone() {
		StreamingFreeResourceEvent clone = (StreamingFreeResourceEvent) super.clone();
		clone.resOwner = null;
		clone.resName = null;
		clone.resAmount = 0;
		return clone;
	...
	public void run() throws RunException {
		...
		if (this.resName.equals("Streaming")) {
			getLogger().fine("cpu amount before freedom: " + ((StreamingPeer) resOwner).getPower());
			((StreamingPeer) this.resOwner).setPower(((StreamingPeer) this.resOwner)
					.getPower()
					+ this.resAmount);
			getLogger().fine("cpu amount after freedom: " + ((StreamingPeer) resOwner).getPower());
		}
