...
/**
 * This class manages the dialog window to set
 * the attributes and any more of all vertices
 * @author Fabrizio Caramia (caramia@ce.unipr.it)
 * @author Mario Sabbatelli (smario@ce.unipr.it) 
 * 
 */

public class VertexPropertiesDialog extends JDialog implements 
ActionListener, ItemListener, ListSelectionListener{
	...
	private String choiceLevel [] = {"Select...", "OFF", "SEVERE" ,"WARNING" ,"INFO" ,"CONFIG" ,"FINE" ,"FINER" ,"FINEST" ,"ALL" };
	...
	private void designDialogPanel(){
		...
		optAttPanel.setBorder(new javax.swing.border.TitledBorder("Optional attributes"));
		...
		if(vert.getElementType()!="Engine" && vert.getElementType()!="Network"){
			
			JPanel paramPanel =new JPanel(new GridLayout(0,1));
			paramPanel.setBorder(new javax.swing.border.TitledBorder("Parameter"));
...
private JComponent paramResAddPanel(){
        ...
        /*if(vert.getSelectParam()){
        	...
        	String name;
        	String value;
        	//ArrayList<String> pName=vert.getParamName();
        	//ArrayList<String> pValue=vert.getParamValue();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand()=="OK"){
			...
			if (vert.getElementType()!="Engine"){
				...
				if(vert.getElementType()!="Network"){
					...
					if(this.checkBoxParam.isSelected()){
						...
						nameList=(ArrayList<String>) Collections.list(modelParamName.elements());
						ArrayList<String> valueList=new ArrayList<String>();
						valueList= (ArrayList<String>) Collections.list(modelParamValue.elements());
				...
				if(vert.getElementType()=="Node"){
					...
					//***** Node resources
					if(this.checkBoxResources.isSelected() && resParamList.size()!=0){
				...
				if(vert.getElementType()=="Event"){
					...
					//check select item
					if(this.checkBoxSListener.isSelected() && !(this.textFieldSchedList.getText().equals("")))
						vert.setSchedListener((String)this.textFieldSchedList.getText(), true);
					
					//check select item
					if(this.checkBoxOneShot.isSelected() && this.comboOneShot.getSelectedIndex()!=0)
						vert.setOneShot(this.comboOneShot.getSelectedItem().toString(), true);
					
					if(this.checkBoxDisLevel.isSelected() && !(this.textFieldDisLevel.getText().equals(""))){
						vert.setDisLevel((String)this.textFieldDisLevel.getText(), true);
				...
				vert.setMaxVT(Float.valueOf(this.textFieldMaxVt.getText()).floatValue());
				vert.setSeed(Integer.valueOf(this.textFieldSeed.getText()).intValue());
				
				//check select item
				if(this.checkBoxKeySS.isSelected() && !(this.textFieldKeySS.getText().equals(""))){
					vert.setKeySS(Integer.valueOf(this.textFieldKeySS.getText()).intValue(), true);
				}
			...
			if(vert.getElementType()!="Network"){
				...
				if(this.checkBoxLog.isSelected()&& this.comboLevel.getSelectedIndex()!=0){
						vert.setLogger(String.valueOf(this.comboLevel.getSelectedItem()), this.textFieldPathPrefix.getText(), true);
				}
			...
			if(modelResParamName.isEmpty() || textFieldResHandler.getText().equals("")){
				JOptionPane.showMessageDialog(basic,
				...
				nameResParam=(ArrayList<String>) Collections.list(modelResParamName.elements());
				valueResParam=(ArrayList<String>) Collections.list(modelResParamValue.elements());
			...
			if(comboResources.getItemCount()==0)
				JOptionPane.showMessageDialog(basic,
					    "No resources to eliminate",
					    "Warning",
					    JOptionPane.WARNING_MESSAGE);
			else{
				
				comboResources.removeItem(comboResources.getItemAt(comboResources.getSelectedIndex()));
			...
			if(this.textFieldResParamName.getText().equals("")){
				...
				modelResParamName.add(posRes,this.textFieldResParamName.getText());
				this.textFieldResParamName.setText("");
				
				
				modelResParamValue.add(posRes,this.textFieldResParamValue.getText());
	...
	public void valueChanged(ListSelectionEvent e) {
		 ...
		 if(lsm.equals(listSelectionModelName))
			 ...
			 listResParamValue.setSelectedIndex(listResParamName.getSelectedIndex());
		 else if(lsm.equals(listSelectionModelResValue))
			 listResParamName.setSelectedIndex(listResParamValue.getSelectedIndex());
