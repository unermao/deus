...
/**
 * A class useful to create a marshall file in xml after validation and
...
public class Marshalling {
	...
	private void configureAutomator(){
		...
		for (Iterator<DeusVertex> graphInfo= this.graph.getVertices().iterator(); graphInfo.hasNext();){
			...
			if(vert.getElementType()=="Engine"){
				...
				for(Iterator<DeusVertex> vertNeighB=vertNeighBCollection.iterator(); vertNeighB.hasNext();){
				...
				for(Iterator<DeusVertex> vertNeighB=vertNeighBCollection.iterator(); vertNeighB.hasNext();){
					...
					if(vertLink.getElementType()=="Event"){
						...
						referencesEventsIst.getReference().add(referenceEventIst);
					}
				...
				for(Iterator<DeusVertex> vertS=vertNeighBCollection.iterator(); vertS.hasNext();){
				...
				if(vert.getSelectResources()){
					...
					Iterator<ResourceParam> itResources= resParamList.iterator();		
					...
					while(itResources.hasNext()){
						...
						Iterator<String> iterName=resPar.getNameResParamList().iterator();
						Iterator<String> iterValue=resPar.getValueResParamList().iterator();
				...
				for(Iterator<DeusVertex> vertS=vertNeighBCollection.iterator(); vertS.hasNext();){
					...
					if(vertLink.getElementType()=="Node"){
						...
						referencesNodesNetIst.getReference().add(referenceNodeNetIst);
	...
	private void marshallAutomator(){
		...
		try {
			...
			if(this.doValidation){
				...
				//SchemaFactory sf = SchemaFactory.newInstance("http://www.w3.org/2001/XMLSchema"); //For Mac Os X with old java version
			    try {
				...
				marshallerAutomator.setEventHandler(new ValidationEventHandler() {
			...
			if(this.doValidation){
				...
				if(n==0)
					DeusUtility.saveVertexPosition(vv, this.filename.getAbsolutePath());
	...
	private FileOutputStream getFileStream(){
		...
		try {
			...
			System.out.println("The problem creating a destination file was "+ e);
		return null;
		}
