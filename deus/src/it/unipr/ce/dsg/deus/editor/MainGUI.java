...
public class MainGUI {
	    ...
	    private String[] typeLayout ={"Static", "DAGLayout", "FRLayout", "ISOMLayout", "CircleLayout","Spring"};
	    ...
	    /** 
	     * Class constructor that instantiate VisualizationViewer, Graph, factory and layout.
	     * Manages the principal graphic panel such as toolbar, right panel and menu.
	     *  
	     */
	    public MainGUI() {
	        ...
	        layout = new StaticLayout<DeusVertex, DeusEdge>(graph, new Dimension(700,550));
	        //Spring Layout
	        ...
	        gm = new EditingModalGraphMouse<DeusVertex, DeusEdge>(vv.getRenderContext(), 
	        ...
	        vv.getRenderContext().setVertexLabelTransformer(new ToStringLabeller<DeusVertex>());
	        vv.getRenderContext().setEdgeLabelTransformer(new ToStringLabeller<DeusEdge>());
	        ...
	        selectLayoutCombo.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(cb.getSelectedItem().toString()=="Spring"){
						...
						Relaxer relaxer = new VisRunner((IterativeContext)springLayout);
	            		relaxer.stop();
	            		relaxer.prerelax();
						
						LayoutTransition<DeusVertex, DeusEdge> lt =
	    					new LayoutTransition<DeusVertex, DeusEdge>(vv, vv.getGraphLayout(),
	    							springLayout	);
						...
						Relaxer relaxer = new VisRunner((IterativeContext)isomLayout);
	            		relaxer.pause();
	...
	private void exitCheck(){
		...
		if(numVert!=0){
			int n = JOptionPane.showConfirmDialog(
				    MainGUI.frame,
				    "The graph will be deleted. Exit?",
				    "Exit",
