...
/**
 * A class that unmarshall the file XML and draw
 * on Graph corresponding instances
	...
	*/
	...
	for(Node nodeXml:nodeList){
		...
		if(nodeXml.getResources()!=null){
			List<Resource> resurceList=nodeXml.getResources().getResource();
	...
	for(Event eventXml:eventList){
		...
		if(eventXml.getEvents()!=null){
			List<Reference> eventsList= eventXml.getEvents().getReference();
			for(Reference eventl:eventsList){
					DeusEdge edge = new DeusEdge();
					graph.addEdge(edge, DeusUtility.getVertexById(eventXml.getId(), events), DeusUtility.getVertexById(eventl.getId(), events));
			}
		}
	}
	...
	for(Process processXml:processList){
		...
		if(logger!=null){
			process.setLogger(logger.getLevel(), logger.getPathPrefix(), true);
		}
		if(processXml.getEvents()!=null){
			List<Reference> eventsList= processXml.getEvents().getReference();
			for(Reference event:eventsList){
				DeusEdge edge = new DeusEdge();
				graph.addEdge(edge, process, DeusUtility.getVertexById(event.getId(), events));
	
			}
		}
		if(processXml.getNodes()!=null){
			List<Reference> nodesList=processXml.getNodes().getReference();
			for(Reference node:nodesList){
				DeusEdge edge = new DeusEdge();
				graph.addEdge(edge, process, DeusUtility.getVertexById(node.getId(), nodes));
			}
		}
	...
	if(networkXml!=null){
		...
		if(networkXml.getNodes()!=null){
			List<Reference> nodeNetList = networkXml.getNodes().getReference();
			for(Reference nodeNet:nodeNetList){
				DeusEdge edge = new DeusEdge();
				if(nodeNet.getNumber()!=null)
					edge.setNumber(nodeNet.getNumber());
				graph.addEdge(edge, network, DeusUtility.getVertexById(nodeNet.getId(), nodes));
			}
		}
	...
	if(engineXml!=null){
		...
		if(logger!=null){
			engine.setLogger(logger.getLevel(), logger.getPathPrefix(), true);
		}
		if(engineXml.getProcesses()!=null){
			List<Reference> processesList= engineXml.getProcesses().getReference();
			for(Reference proc:processesList){
				DeusEdge edge = new DeusEdge();
				graph.addEdge(edge, engine, DeusUtility.getVertexById(proc.getId(), processes));
	...
	Layout<DeusVertex, DeusEdge> frLayout = new FRLayout<DeusVertex, DeusEdge>(graph);
	frLayout.setSize(visView.getSize());
	
	MainGUI.layout= new StaticLayout<DeusVertex, DeusEdge>(graph, frLayout);
	//Layout<DeusVertex, DeusEdge> staticL= new StaticLayout<DeusVertex, DeusEdge>(graph, frLayout);
