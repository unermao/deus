...
public class Node {
	...
	public String oneLineString(){
		...
		for(int index = 0 ; index < this.nodeParameterList.size(); index++)
		{
			...
			line = line + " " + nodeParameter.getParamName() + " " + nodeParameter.getInitialValue() + " " + nodeParameter.getFinalValue() + " " + nodeParameter.getStepValue(); 
		}
		
		for(int index = 0 ; index < this.nodeResourceList.size(); index++)
		{
			NodeResource nodeResource = this.nodeResourceList.get(index);
			line = line + " " + nodeResource.getHandlerName() + " " + nodeResource.getResParamValue() +  " " + nodeResource.getInitialValue() + " " + " " + nodeResource.getFinalValue() + " " + nodeResource.getStepValue(); 
		}
	 ...
	 *		<finalValue>3.0</finalValue>
	 *		<stepValue>0.5</stepValue>
	 *	</paramName>
	 *	<paramName name="downloadSpeed">
	 *		<initialValue>7.0</initialValue>
	 *		<finalValue>4.5</finalValue>
	 *		<stepValue>0.5</stepValue>
	 *	</paramName>
	 *	<resourceParamName handlerName="it.unipr.ce.dsg.deus.impl.resource.AllocableResource" resParamValue="MaxAcceptedConnection" >
	 *		<initialValue>4.5</initialValue>
	 *		<finalValue>7.0</finalValue>
	 *		<stepValue>0.5</stepValue>
	 *	</resourceParamName>
	 * </node>	
	 * @return
	...
	public String getXMLTag(){
		...
		for(int index = 0 ; index < this.nodeParameterList.size(); index++)
		{
			...
			tag = tag + "\n\t\t" + "<initialValue>"+ nodeParameter.getInitialValue() + "</initialValue>";
			tag = tag + "\n\t\t" + "<finalValue>"+ nodeParameter.getFinalValue() + "</finalValue>";
			tag = tag + "\n\t\t" + "<stepValue>"+ nodeParameter.getStepValue() + "</stepValue>";
			tag = tag + "\n\t" + "</paramName>" + "\n\t";
		}
		...
		{
			NodeResource nodeResource = this.nodeResourceList.get(index);
			tag = tag + "<resourceParamName handlerName=\"" + nodeResource.getHandlerName() + "\" resParamValue=\"" + nodeResource.getResParamValue() +"\">";
			tag = tag + "\n\t\t" + "<initialValue>"+ nodeResource.getInitialValue() + "</initialValue>";
			tag = tag + "\n\t\t" + "<finalValue>"+ nodeResource.getFinalValue() + "</finalValue>";
			tag = tag + "\n\t\t" + "<stepValue>"+ nodeResource.getStepValue() + "</stepValue>";
			tag = tag + "\n\t" + "</resourceParamName>" + "\t";
		}
